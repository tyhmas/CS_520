...
public class MineSweeper {
	...
	public static void main(String[] args) {
		System.out.println("correct rate:"+MineSweeperTest(10000, 10,10, 0.2, 0.1));
		/*currGame test = new currGame(10,10,0.2,0.1);
		test.print();
		test.printCurr();
	...
	public static double MineSweeperTest(int its, int dim_i,int dim_j, double p, double greatProb) {
		int win = 0;
		int tmpIts = its;
		while(tmpIts!=0) {
			System.out.println("its: "+tmpIts+"/"+its);
			tmpIts--;
			currGame test = new currGame(dim_i,dim_j,p, greatProb);
	...
	public static boolean diggedAll(currGame currGame) {
		for(int i=0;i<currGame.dim_i;i++) {
			for(int j=0;j<currGame.dim_j;j++) {
				if(currGame.curr[i][j].type=='u'&&currGame.game[i][j].type!='m')//
					return false;
			}
		}
		return true;
	...
	public static void updateNeighborHelper(currGame currGame, int x, int y) {
				//System.out.println("updating according the clue in ("+x+", "+y+")");
