...
public class MineSweeper {
	...
	public static void main(String[] args) {
		System.out.println("correct rate:"+MineSweeperTest(10000, 5,5, 0.2,0.3));
		/*currGame test = new currGame(5,5,0.1,0.5);
		test.print();
		boolean result = winMineSweeper(test, 0, 0);
		System.out.println("win?"+result);*/
	}
	
	public static double MineSweeperTest(int its, int dim_i,int dim_j, double p, double uncertainP) {
		int win = 0;
		int tmpIts = its;
		while(tmpIts!=0) {
			System.out.println("its: "+tmpIts+"/"+its);
			tmpIts--;
			currGame test = new currGame(dim_i,dim_j,p, uncertainP);
			boolean result = winMineSweeper(test, 0, 0);
			if(result==true) {
				win++;
			}
	...
	public static boolean winMineSweeper(currGame currGame, int x, int y) {
		while(!currGame.over) {
			...
			//System.out.println("after mark mine");
			//currGame.printCurr();
		}
		return currGame.win;
	}
	...
	public static void digXY(currGame currGame, int x, int y) {
		...
		if(currGame.game[x][y].type=='c'&&currGame.game[x][y].clue!=0) {
			...
			if(!currGame.existed(currGame.margin, currGame.curr[x][y])) {
				...
				//System.out.println("hit margin: ("+x+", "+y+")");
			}
			return;
		}
		...
		if(currGame.game[x][y].type=='c'&&currGame.game[x][y].clue==0) {
			//System.out.println("internal: ("+x+", "+y+")");
			currGame.curr[x][y].type='c';
	...
	public static Cell findNext(currGame currGame) {
		...
		for(int i=0;i<currGame.dim_i;i++) {
			for(int j=0;j<currGame.dim_j;j++) {
				if(currGame.curr[i][j].type=='u') {
					if(currGame.curr[i][j].prob<=minProb){
						tmp.add(currGame.curr[i][j]);
					}
				}
			}
		}
		/*for(int i=0;i<tmp.size();i++) {//for part 5-1
			System.out.println("minProb list: ("+i+"th) "+tmp.get(i).prob+", "+tmp.get(i).info);
		}*/
		
		for(int i=0;i<tmp.size();i++) {//for part 5-1
			if(tmp.get(i).info>maxInfo) {
				maxInfo=tmp.get(i).info;
			}
		}
		...
		for(int i=0;i<tmp.size();i++) {//for part 5-1
			if(tmp.get(i).info>=maxInfo) {
				//System.out.println("minProb & maxInfo: ("+i+"th) ("+tmp.get(i).x+","+tmp.get(i).y+") "+tmp.get(i).prob+", "+tmp.get(i).info);
				return tmp.get(i);
			}
		}
	...
	public static void updateNeighbor(currGame currGame) {
		...
		for(int i=0;i<currGame.margin.size();i++) {
			if(currGame.margin.get(i).clue!=-2)//exclude "-2," no clue cells
	...
	public static void updateInfo(currGame currGame) {
		for(int i=0;i<currGame.dim_i;i++) {
			for(int j=0;j<currGame.dim_j;j++) {
				updateInfoHelper(currGame, i, j);
			}
		}
	}
	
	public static void updateInfoHelper(currGame currGame,int x, int y) {
		int neighborsLeft=0;
		//up left
		if(x-1>=0&&y-1>=0) {
			if(currGame.curr[x-1][y-1].type=='u')
		...
		//System.out.println("neighborsLeft:"+neighborsLeft);
		currGame.curr[x][y].info = currGame.uncertainP*(double)neighborsLeft;
		//System.out.println("changed info:"+currGame.curr[x][y].info);
	}
